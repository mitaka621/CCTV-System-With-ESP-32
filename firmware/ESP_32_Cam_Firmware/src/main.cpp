...
static void onCameraCommand(CameraCommand command, const uint8_t *payload, size_t payloadLen)
{
  switch (command)
  {
  case CameraCommand::ResetSecurityAlarm:
    DEBUG_PRINT("Command: reset security alarm");
    sensor_hub::resetMotion();
    buzzer::setActive(false);
    break;
  case CameraCommand::ActivateBuzzerAlarm:
    DEBUG_PRINT("Command: trigger security alarm");
    buzzer::setActive(true);
    break;
  case CameraCommand::SaveNewConfig:
...
static void onChannelMessage(const uint8_t *data, size_t len)
{
  if (len < 2 || data[0] != DEVICE_COMMAND_VERSION)
  {
    DEBUG_PRINT("Command: unsupported message envelope");
    return;
  }

  onCameraCommand((CameraCommand)data[1], data + 2, len - 2);
}
...
void setup()
{
  ...
  sensor_hub::begin();
  buzzer::begin();
  data_channel::SetMessageHandler(onChannelMessage);
